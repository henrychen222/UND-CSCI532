...
int main(int argc, char **argv) {
    ...
    for (int steps = 0; steps < time_steps; steps++)
    {
        ...
        for (int i = 0; i < my_height; ++i)
            my_block[i + 1][0] = from_west[i];
        for (int i = 0; i < my_height; ++i)
            my_block[i + 1][my_width + 1] = from_east[i];
        for (int i = 0; i < my_width; ++i)
            my_block[0][i + 1] = from_south[i];
        for (int i = 0; i < my_width; ++i)
            my_block[my_height + 1][i + 1] = from_north[i];
        ...
        //MPI_Gather(my_result, my_width*my_height, MPI_DOUBLE, values, )
